...
export const getGruaById = async (req, res) => {
    ...
    if (idGrua){
        ...
        return res.status(HTTP_STATUS.BAD_REQUEST)
        .json({msg: MESSAGES.BAD_REQUEST, content: {"idGrua": idGrua}});
    }
};
...
export const deleteGrua = async (req,res) => {
    ...
    if (idGrua){
        ...
        return res.status(HTTP_STATUS.BAD_REQUEST).json({msg: MESSAGES.BAD_REQUEST, content: {'idGrua':idGrua}});
    }
};
...
export const updateGruaById = async (req, res) => {
    ...
    if (idCorralon && idGruaTipo && matricula && idGrua) {
        try {
            const pool = await getConection();
